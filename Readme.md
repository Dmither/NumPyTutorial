...
аргумент `ndmin=n` задає кількість вимірів  
`np.arange([start, ]end[, step)` повертає масив значень за ранжуванням  
...
зміна даних у вихідному масиві або представленнях змінюють всюди  
...
Універсальні ф-ї ufuncs дозволяють оперувати об'єктами ndarray.  
Реалізують векторизацію, що набагато швидше ітерації елементів.  
Приймають додаткові аргументи:  
`where` логічний масив або умова, де мають відбуватися операції;  
`dtype` визначає тип повернення елементів;  
`out` визідний масив, куди слід скопіювати повернуте значення  
**Векторизація** - перетворення ітеративних операторів у векторну операцію. Процесори оптимізовані для таких операцій.  
Тип ufunc: `type(np.add)` повертає `<class 'numpy.ufunc'>`  
`np.ufunc` псевдонім типу (type(np.add) == np.ufunc)

**Створення власної ufunc**  
\- визначити користувацьку функцію засобами python;  
\- додати до ufunc `np.frompyfunc(function, inputs, outputs), де  
`function` - назва функції,  
`inputs` - кількість вхідних аргументів (масивів),  
`outputs` - кількість вихідних масивів.
...
**Проста арифметика**  
дозволяє проводити арифметичні операції між елементами ndarray  
`np.add(x, y)` повертає масив сум відповідних елементів масивів  
`np.subtract(x, y)` повертає різниці відповідних елементів  
`np.multiply(x, y)` повертає добуток відповідних елементів  
`np.divide(x, y)` повертає частку елементів першого на другого  
`np.power(x, y)` повертає елемент першого в степені другого  
`np.mod(x, y)`, `np.remainder(x, y)` повертають залишок ділення  
`np.divmod(x, y)` повертає кортеж масивів divide і mod  
`np.absolute(x)`, `np.abs(x)` повертає масив модулів елементів

**Округлення десяткових чисел**  
Методи приймають і повертають масив, n - к-ть десяткових знаків  
`np.trunc(a)`, `np.fix(a)` повертає усічення до найближчого до 0  
`np.around(a, n)` повертає округлення до найближчого значення  
`np.floor(a, n)` повертає округлення до меншого значення  
`np.ceil(a, n)` повертає округлення до більшого значення  

**Логарифми**  
`np.log2(a)` повертає масив логарифмів за основою 2  
`np.log10(a)` повертає масив логарифмів за основою 10  
`np.log(a)` повертає масив логарифмів з натуральною основою (e)  
NumPy не надає ф-й для обчислення логарифмів за іншими основами,  
але це можна реалізувати за допомогою frompyfunc() та math.log():  
`nplog = np.frompyfunc(math.log, 2, 1)`

**Суми, добутки**  
`np.sum(a)` повертає суму всіх ел-в, може приймати список масивів  
при додаванні аргументу `axis` (int) підсумовує за віссю  
`np.cumsum(a)` повретає масив сум з накопиченням ([1, 3, 6, 10])*  
`np.prod(a)` повертає добуток всіх ел-в*  
`np.cumprod(a)` повертає масив добутків з накопиченням*  
*можуть приймати список масивів, має необов'язковий `axis`  

**Дискретна різниця**  
означає віднімання двох послідовнх елементів  
`np.diff(a, n=1)` повертає масив різниць пар сусідніх елементів  
`np.diff([10, 15, 25, 5])` поверне `[5 10 -20]`  
аргумент `n` визначає кількісь ітерацій дифрізниці  
`np.diff(a, n=2)` поверне `[5, -30]`, `n=0` - вихідний масив  

**Найменше спільне кратне, найбільший спільний дільник**  
`np.lcm(x, y)` повертає найменше спільне кратне двох чисел  
`np.gcd(x, y)` повертає найбільший спільний дільник вдох чисел  
`np.lcm.reduce(a)` повертаж НСК чисел масиву*  
`np.gcd.reduce(a)` повертаж НСД чисел масиву*  
*можуть приймати список масивів, має необов'язковий `axis`  

**Тригонометричні функції**  
`np.sin()`, `np.cos()`, `np.tan()` приймають значення в радіанах,  
повертають значення тригонометричних ф-й*  
`np.deg2rad()` перетворює градуси в радіани*  
`np.rad2deg()` перетворює радіани в градуси*  
`np.arcsin()`, `np.arccos()`, `np.arctan()` приймають значення  
тригонометричних ф-й*, повертають радіанні значення  
*приймають одне значення або масив  
`np.hypot(base, perp)` повертає гіпотенузу прямокутного трик-ка  
`sinh`, `cosh`, `tanh`, `arcsinh`, `arccosh`, `arctanh`  
відповідні гіперболічні функції  

**Операції з наборами**  
`np.unique(a)` повертає множину (унікальних значень) 1d масиву  
`np.union1d()`* повертає об'єднання двох множин  
`np.intersect1d()`** повертає перетин двох множин  
`np.setdiff1d()`** повертає різницю (значення 1 без 2)  
`np.setxor1d()`** повертає симетричну різницю (1 і 2 але не обох)  
